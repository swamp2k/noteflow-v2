...
export async function notesHandler(request, env, ctx, url, path, method, userId, origin) {
  ...
  if (path === "/api/notes" && method === "POST") {
    const body=await request.json(); const{content="",visibility="PRIVATE",created_at,updated_at,tags:bodyTags,is_task,due_date,priority}=body;
    ...
    await env.DB.prepare("INSERT INTO notes (id,user_id,content,visibility,created_at,updated_at,is_task,due_date,priority) VALUES (?,?,?,?,?,?,?,?,?)").bind(id,userId,content,visibility,ca,ua,taskFlag,dueVal,prioVal).run();
