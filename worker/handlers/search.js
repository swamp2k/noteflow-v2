...
export async function searchHandler(request, env, ctx, url, path, method, userId, origin) {
  if(path==="/api/search"&&method==="POST"){const{q,tag}=await request.json();if(!q||q.trim().length<2)return json({notes:[]},200,origin);const term=`%${q.trim()}%`;const notesSql=tag?"SELECT n.id,n.content,n.created_at,n.updated_at,n.visibility,n.pinned,n.archived,GROUP_CONCAT(nt.tag) as tags_csv,NULL as matched_file FROM notes n LEFT JOIN note_tags nt ON nt.note_id=n.id INNER JOIN note_tags tf ON tf.note_id=n.id AND tf.tag=? WHERE n.user_id=? AND n.archived=0 AND n.content LIKE ? GROUP BY n.id ORDER BY n.created_at DESC LIMIT 50":"SELECT n.id,n.content,n.created_at,n.updated_at,n.visibility,n.pinned,n.archived,GROUP_CONCAT(nt.tag) as tags_csv,NULL as matched_file FROM notes n LEFT JOIN note_tags nt ON nt.note_id=n.id WHERE n.user_id=? AND n.archived=0 AND n.content LIKE ? GROUP BY n.id ORDER BY n.created_at DESC LIMIT 50";const docsSql=tag?"SELECT n.id,n.content,n.created_at,n.updated_at,n.visibility,n.pinned,n.archived,GROUP_CONCAT(nt.tag) as tags_csv,a.filename as matched_file FROM document_index di JOIN attachments a ON a.id=di.attachment_id JOIN notes n ON n.id=a.note_id LEFT JOIN note_tags nt ON nt.note_id=n.id INNER JOIN note_tags tf ON tf.note_id=n.id AND tf.tag=? WHERE n.user_id=? AND n.archived=0 AND di.text_content LIKE ? GROUP BY n.id ORDER BY n.created_at DESC LIMIT 20":"SELECT n.id,n.content,n.created_at,n.updated_at,n.visibility,n.pinned,n.archived,GROUP_CONCAT(nt.tag) as tags_csv,a.filename as matched_file FROM document_index di JOIN attachments a ON a.id=di.attachment_id JOIN notes n ON n.id=a.note_id LEFT JOIN note_tags nt ON nt.note_id=n.id WHERE n.user_id=? AND n.archived=0 AND di.text_content LIKE ? GROUP BY n.id ORDER BY n.created_at DESC LIMIT 20";const{results:nr}=await env.DB.prepare(notesSql).bind(...(tag?[tag,userId,term]:[userId,term])).all();const{results:dr}=await env.DB.prepare(docsSql).bind(...(tag?[tag,userId,term]:[userId,term])).all();const seen=new Set();const merged=[];for(const n of[...nr,...dr]){if(!seen.has(n.id)){seen.add(n.id);merged.push({...n,tags:n.tags_csv?n.tags_csv.split(","):[],tags_csv:void 0});}}if(merged.length>0){const ids=merged.map(n=>n.id);const ph=ids.map(()=>"?").join(",");const{results:atts}=await env.DB.prepare(`SELECT * FROM attachments WHERE note_id IN (${ph})`).bind(...ids).all();const am={};for(const a of atts){if(!am[a.note_id])am[a.note_id]=[];am[a.note_id].push(a);}for(const n of merged)n.attachments=am[n.id]||[];}return json({notes:merged},200,origin);}
