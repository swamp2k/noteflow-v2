...
[ tags (optional) ]
```
- Subject badge, due date chip, and notification chip are **display only** — no click handlers, no inline editing on the card.
...
- No Edit/Archive action buttons on the card. No inline editing — all edits happen in the detail modal.
